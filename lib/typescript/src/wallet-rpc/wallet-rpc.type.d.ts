...
import type { alias_rpc_details, API_RETURN_CODE, asset_balance_entry, asset_descriptor_base, asset_descriptor_with_id, contract_private_details, contracts_array, data_for_external_asset_signing_tx, htlc_entry_info, ionic_swap_proposal_info, JSON_RPC_ERROR_CODE, mining_history, offer_details_ex, payment_details, transfer_destination, tx_service_attachment, wallet_entry_info, wallet_provision_info, WALLET_RPC_ERROR_CODE, wallet_transfer_info, wallet_transfer_info_old } from '../entities';
...
export type INVOKE_RPC_TRANSFER_ASSET_OWNERSHIP_RESPONSE = {
    /** Status of the call */
    status: API_RETURN_CODE.BAD_ARG_INVALID_ADDRESS | API_RETURN_CODE.OK | `Couldn't find asset_id "${string}" in own assets list` | 'Failed to get asset info from daemon' | (string & {});
...
type WalletMethodErrors = TypedJSON<JSONRpcReturnCodeApiReturnCode> | TypedJSON<JSONRpcFailedResponse<ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, API_RETURN_CODE.BUSY>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<JSON_RPC_ERROR_CODE.PARSE_ERROR, 'Parse error'>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<JSON_RPC_ERROR_CODE.INVALID_REQUEST, 'Invalid Request'>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<JSON_RPC_ERROR_CODE.INVALID_PARAMS, 'Invalid params'>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<WALLET_RPC_ERROR_CODE.DAEMON_IS_BUSY, `WALLET_RPC_ERROR_CODE_DAEMON_IS_BUSY${string}`>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<WALLET_RPC_ERROR_CODE.NOT_ENOUGH_MONEY, `WALLET_RPC_ERROR_CODE_NOT_ENOUGH_MONEY${string}`>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<WALLET_RPC_ERROR_CODE.GENERIC_TRANSFER_ERROR>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<WALLET_RPC_ERROR_CODE.GENERIC_TRANSFER_ERROR, `WALLET_RPC_ERROR_CODE_GENERIC_TRANSFER_ERROR${string}`>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, 'WALLET_RPC_ERROR_CODE_UNKNOWN_ERROR'>>> | TypedJSON<JSONRpcFailedResponse<ErrorObj<API_RETURN_CODE.UNINITIALIZED, `${API_RETURN_CODE.INTERNAL_ERROR} ${string}` | API_RETURN_CODE.INTERNAL_ERROR>>> | API_RETURN_CODE.WALLET_WRONG_ID;
type WalletMethod<Params, Result, Errors = never> = (instance_id: number, params: TypedJSON<Params>) => TypedJSON<JSONRpcSuccessfulResponse<Result>> | TypedJSON<JSONRpcFailedResponse<Errors>> | WalletMethodErrors;
type WalletAsyncMethod<Params, Result, Errors = never> = (instance_id: number, params: TypedJSON<Params>) => Promise<TypedJSON<JSONRpcSuccessfulResponse<Result>> | TypedJSON<JSONRpcFailedResponse<Errors>> | WalletMethodErrors>;
export interface IWalletRpc extends WalletRpc {
    ...
    transfer: WalletAsyncMethod<INVOKE_RPC_TRANSFER_REQUEST, INVOKE_RPC_TRANSFER_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, `Given fee is too low: ${number}, minimum is: ${number}`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `invalid encoded payment id: ${string}, hex-encoded string was expected`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'Second wrap entry not supported in transactions'> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, `WALLET_RPC_ERROR_CODE_WRONG_ADDRESS: ${string}`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `embedded payment id: ${string} conflicts with previously set payment id: ${string}`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `payment id ${string} is invalid and can't be set`>>;
    ...
    get_payments: WalletAsyncMethod<INVOKE_RPC_GET_PAYMENTS_REQUEST, INVOKE_RPC_GET_PAYMENTS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `invalid payment id given: '${string}', hex-encoded string was expected`>>;
    /** Gets list of incoming transfers by a given multiple payment_ids */
    get_bulk_payments: WalletAsyncMethod<INVOKE_RPC_GET_BULK_PAYMENTS_REQUEST, INVOKE_RPC_GET_BULK_PAYMENTS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `invalid payment id given: '${string}', hex-encoded string was expected`>>;
    /** Generate integrated address */
    make_integrated_address: WalletMethod<INVOKE_RPC_MAKE_INTEGRATED_ADDRESS_REQUEST, INVOKE_RPC_MAKE_INTEGRATED_ADDRESS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `invalid payment id given: '${string}', hex-encoded string was expected`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `given payment id is too long: '${string}'`>>;
    /** Decode integrated address */
    split_integrated_address: WalletMethod<INVOKE_RPC_SPLIT_INTEGRATED_ADDRESS_REQUEST, INVOKE_RPC_SPLIT_INTEGRATED_ADDRESS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, `invalid integrated address given: '${string}'`>>;
    /** Tries to transfer all coins with amount below the given limit to the given address */
    sweep_below: WalletMethod<INVOKE_RPC_SWEEP_BELOW_REQUEST, INVOKE_RPC_SWEEP_BELOW_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `Invalid payment id: ${string}`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, `Invalid address: ${string}`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_PAYMENT_ID, `address ${string} has integrated payment id ${string} which is incompatible with payment id ${string} that was already assigned to this transfer`> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, `Given fee is too low: ${string}, minimum is: ${string}`>>;
    /** Return information about wallet's pre-zarcanum era outputs */
    get_bare_outs_stats: WalletMethod<INVOKE_RPC_GET_BARE_OUTS_STATS_REQUEST, INVOKE_RPC_GET_BARE_OUTS_STATS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, 'operation cannot be performed in watch-only wallet'> | ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, 'get_bare_unspent_outputs_stats failed'>>;
    /** Execute transactions needed to convert all bare(pre-zarcanum) outputs to post-zarcanum outputs */
    sweep_bare_outs: WalletMethod<INVOKE_RPC_SWEEP_BARE_OUTS_REQUEST, INVOKE_RPC_SWEEP_BARE_OUTS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, 'operation cannot be performed in watch-only wallet'> | ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, 'get_bare_unspent_outputs_stats failed'>>;
    /** Sign transaction with the wallet's keys */
    sign_transfer: WalletMethod<INVOKE_RPC_SIGN_TRANSFER_REQUEST, INVOKE_RPC_SIGN_TRANSFER_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'tx_unsigned_hex is invalid'>>;
    /** Relay signed transaction over the network */
    submit_transfer: WalletMethod<INVOKE_RPC_SUBMIT_TRANSFER_REQUEST, INVOKE_RPC_SUBMIT_TRANSFER_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'tx_unsigned_hex is invalid'>>;
    ...
    register_alias: WalletAsyncMethod<INVOKE_RPC_REGISTER_ALIAS_REQUEST, INVOKE_RPC_REGISTER_ALIAS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, 'WALLET_RPC_ERROR_CODE_WRONG_ADDRESS'> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, 'WALLET_RPC_ERROR_CODE_WRONG_ADDRESS - Wrong alias name'>>;
    /** Update an alias details/transfer alias ownership */
    update_alias: WalletAsyncMethod<INVOKE_RPC_UPDATE_ALIAS_REQUEST, INVOKE_RPC_UPDATE_ALIAS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, 'WALLET_RPC_ERROR_CODE_WRONG_ADDRESS'> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, 'WALLET_RPC_ERROR_CODE_WRONG_ADDRESS - Wrong alias name'>>;
    ...
    marketplace_push_offer: WalletMethod<INVOKE_RPC_MARKETPLACE_PUSH_OFFER_REQUEST, INVOKE_RPC_MARKETPLACE_PUSH_OFFER_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'fee is too low'>>;
    /** Updates existing offer that this wallet created, and publish updated version on the blockchain */
    marketplace_push_update_offer: WalletMethod<INVOKE_RPC_MARKETPLACE_PUSH_UPDATE_OFFER_REQUEST, INVOKE_RPC_MARKETPLACE_PUSH_UPDATE_OFFER_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'fee is too low'>>;
    /** Cancel existing offer that this wallet created */
    marketplace_cancel_offer: WalletMethod<INVOKE_RPC_MARKETPLACE_CANCEL_OFFER_REQUEST, INVOKE_RPC_MARKETPLACE_CANCEL_OFFER_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'fee is too low'>>;
    /** Generates ionic swap proposal according to details provided in request */
    ionic_swap_generate_proposal: WalletAsyncMethod<INVOKE_RPC_IONIC_SWAP_GENERATE_PROPOSAL_REQUEST, INVOKE_RPC_IONIC_SWAP_GENERATE_PROPOSAL_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, 'WALLET_RPC_ERROR_CODE_WRONG_ADDRESS'> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ADDRESS, 'WALLET_RPC_ERROR_CODE_WRONG_ADDRESS - integrated address is noit supported yet'> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'WALLET_RPC_ERROR_CODE_WRONG_ARGUMENT - Error creating proposal'>>;
    /** Reads hex-encoded ionic swap proposal info, generated by other user and addressed to this wallet */
    ionic_swap_get_proposal_info: WalletMethod<INVOKE_RPC_IONIC_SWAP_GET_PROPOSAL_INFO_REQUEST, INVOKE_RPC_IONIC_SWAP_GET_PROPOSAL_INFO_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'WALLET_RPC_ERROR_CODE_WRONG_ARGUMENT - failed to parse template from hex'> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'WALLET_RPC_ERROR_CODE_WRONG_ARGUMENT - get_ionic_swap_proposal_info'>>;
    /** This essential command actually execute proposal that was sent by counter party */
    ionic_swap_accept_proposal: WalletMethod<INVOKE_RPC_IONIC_SWAP_ACCEPT_PROPOSAL_REQUEST, INVOKE_RPC_IONIC_SWAP_ACCEPT_PROPOSAL_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'WALLET_RPC_ERROR_CODE_WRONG_ARGUMENT - failed to parse template from hex'> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'WALLET_RPC_ERROR_CODE_WRONG_ARGUMENT - failed to accept_ionic_swap_proposal()'>>;
    ...
    deploy_asset: WalletAsyncMethod<INVOKE_RPC_ASSETS_DEPLOY_REQUEST, INVOKE_RPC_ASSETS_DEPLOY_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, 'asset ticker or full_name is invalid'>>;
    ...
    send_ext_signed_asset_tx: WalletMethod<INVOKE_RPC_ASSET_SEND_EXT_SIGNED_TX_REQUEST, INVOKE_RPC_ASSET_SEND_EXT_SIGNED_TX_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, "finalized_tx couldn't be deserialized"> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, "unsigned_tx doesn't match finalized_tx"> | ErrorObj<WALLET_RPC_ERROR_CODE.WRONG_ARGUMENT, `expected_tx_id mismatch, real tx id is ${string}`>>;
    ...
    mw_get_wallets: WalletMethod<INVOKE_RPC_MW_GET_WALLETS_REQUEST, INVOKE_RPC_MW_GET_WALLETS_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, 'WALLET_RPC_ERROR_CODE_UNKNOWN_ERROR'>>;
    /** Select current active wallet */
    mw_select_wallet: WalletMethod<INVOKE_RPC_MW_SELECT_WALLET_REQUEST, INVOKE_RPC_MW_SELECT_WALLET_RESPONSE, ErrorObj<WALLET_RPC_ERROR_CODE.UNKNOWN_ERROR, 'WALLET_RPC_ERROR_CODE_UNKNOWN_ERROR'>>;
...
export {};
//# sourceMappingURL=wallet-rpc.type.d.ts.map